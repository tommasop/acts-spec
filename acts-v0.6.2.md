...
### 1.5 Changes from v0.6.0

| Area | Change |
|------|--------|
| Task Definition | Tasks MUST include `description` — what the task does |
| Task Definition | Tasks MUST include `acceptance_criteria` — definition of done |
| Task Approval | Each task MUST be explicitly approved during story-init |
| Task Approval | Four approval options: Approve / Modify / Rewrite / Custom |
| Task Approval | Modify/Rewrite/Custom choices require explanation |
| Schema | New `description` field on task (required) |
| Schema | New `acceptance_criteria` field on task (array, required) |
| Operations | story-init Step 5a — Present each task for approval with select interface |
| Operations | story-init Step 5b — Loop until all tasks approved |

### 1.6 Changes from v0.6.1

| Area | Change |
|------|--------|
| Code Review | GitHuman is now the default review provider (was lazygit) |
| Code Review | `task-review` operation rewritten with multi-provider support |
| Code Review | Auto-detection: GitHuman → lazygit → manual fallback |
| Code Review | New `ACTS_REVIEW_PROVIDER` environment variable for provider selection |
| Code Review | Manual review enhanced with summary/full mode (`ACTS_REVIEW_MODE`) |
| Code Review | Manual review includes Conventional Comments format support |
| Code Review | **CRITICAL FIX**: Review GATE now blocks in ALL modes (was returning immediately) |
| Code Review | Review now reads from `/dev/tty` to force interactive input |
| Code Review | Visual "🔒 REVIEW GATE ACTIVE" banners to remind agent to stop |
| Code Review | **CRITICAL FIX**: Added clear messaging when agent reaches review gate |
| Code Review | task-start now shows reminder about how to trigger review on completion |
| Code Review | Review cannot be bypassed - agent must explicitly call task-start with action=complete |
| Code Review | **NEW**: Complete GitHuman workflow with proper menu-based approval |
| Code Review | **NEW**: GitHuman flow: launch → menu → review in browser → select option → export → kill server |
| Code Review | **CRITICAL FIX**: Fallback file-based approval when `/dev/tty` unavailable (agent environment) |
| Code Review | Added `can_read_interactively()` and `read_with_fallback()` functions |
| Code Review | File watcher polls `.story/reviews/active/{task}-approval.txt` for approval |
| GitHuman | Added `run` command to provider configuration |
| Install | `install-acts.sh` always downloads latest spec from GitHub |
| Update | `acts-update` fetches latest version from GitHub instead of local files |
| Update | Fixed logging to stderr to prevent output capture issues |
...
```json
{
  ...
  "$defs": {
    "task": {
      "type": "object",
      "required": ["id", "title", "description", "acceptance_criteria", "status", "assigned_to", "files_touched", "depends_on"],
      "properties": {
        "id": {
          "type": "string",
        ...
        "description": {
          "type": "string",
          "description": "What this task does — implementation details and approach."
        },
        "acceptance_criteria": {
          "type": "array",
          "items": { "type": "string" },
          "description": "Concrete, testable definitions of done for this task."
        },
        "status": {
          "type": "string",
...
### <TASK_ID> — <title>
- **Description:** <what this task does — implementation details>
...
- **Description** — what the task does, implementation approach, key decisions
- **Acceptance Criteria** — numbered list of concrete, testable definitions of done
...
- Logs: stderr
- Exit Codes: 0 (success), 2 (changes_requested), 1 (error)
...
4. Decompose the spec into tasks. Write `.story/plan.md`:
   ...
   - Each task gets: ID, title, description, dependencies, likely files,
     context priority (1-5), acceptance criteria

5. **TASK APPROVAL LOOP** — Present each task for explicit approval:

   For each task in the plan, display:
   ```
   Task <TASK_ID>: <title>
   ...
   **GATE: task-approval**
   ...
   - STOP and wait for explanation
   - Ask: "Please explain what changes are needed:"
   - After receiving explanation, update the task
   - Re-present the updated task for approval
   - Loop until "Approve" is selected

   Repeat for all tasks until every task is approved.

6. Initialize `.story/state.json`:
   - `acts_version`: `0.6.2`
   - `status`: `ANALYSIS`
   - `spec_approved`: `false`
...
7. Commit all tracker files:
   `docs(<story_id>): initialize ACTS tracker`

8. Instruct the developer to review `spec.md` and `plan.md` with
...
- Each task MUST have a `description` field explaining what it does.
- Each task MUST have `acceptance_criteria` — numbered list of testable criteria.
...
**Example: GitHuman Provider (Default)**
...
```json
{
  ...
  "commands": {
    ...
    "serve": { "cmd": "githuman", "args": ["serve", "--port", "3847", "--no-open"] },
    "run": { "cmd": "githuman", "args": ["run", "--port", "3847"] },
    "status": { "cmd": "githuman", "args": ["list", "--status", "--json"] },
    "export": { "cmd": "githuman", "args": ["export", "last", "--output", "{output_path}"] }
  }
}
```
...
**GitHuman Review Flow (v0.6.2+ Default):**

1. **Launch**: Agent starts `githuman serve --port 3847`
2. **Menu**: Agent displays URL and review menu with 3 options
3. **Review**: Developer opens browser to `http://127.0.0.1:3847` and reviews
4. **Submit**: Developer closes review in GitHuman interface
5. **Select**: Developer chooses from agent menu (Approved/Changes/Cancel)
6. **Export**: Agent exports review to `.story/reviews/active/<task>-githuman.md`
7. **Stop**: Agent kills GitHuman server
8. **Artifact**: Agent creates review record and archives it
9. **Loop**: If changes requested, task stays IN_PROGRESS, loop back to step 1
...
- Format: `<task_id>-review.md` (agent artifact)
- Format: `<task_id>-githuman-export.md` (GitHuman export)
...
- Triggered when `task-start` is called with `action=complete`
- **HARD GATE**: Blocks until explicit developer approval
- Supports multiple providers with automatic fallback
- Provider selection: `ACTS_REVIEW_PROVIDER=githuman|lazygit|manual`
- Review mode: `ACTS_REVIEW_MODE=summary|full` (for manual review)
- Exports review to `.story/reviews/active/`
- Returns status: `approved`, `changes_requested`, or `cancelled`
- If `changes_requested`: Task stays IN_PROGRESS, developer must re-run after fixes

**Menu Options (All Providers):**
- `[1] Approved` - Changes accepted, task can complete
- `[2] Changes Requested` - Needs modifications, task stays in progress  
- `[3] Cancel` - Abort review, task stays in progress
