...
5. **v0.4.0:** Generic CLI-based code review interface (critique primary)
...
└── review-providers/              # v0.4.0: Provider configs
    └── critique.json             # critique CLI configuration
```
...
| `task-review` | After staging, before commit | Code Review report (critique interface) |
...
2. **Code Review Gate** (`task-review`)
    - Starts critique review server: `bunx critique --staged`
...
- **critique** must be installed: `bun install -g critique`
...
- **Migration:** Install critique, enable when ready
