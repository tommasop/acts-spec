        ...
        new_acts_config.update(
            {
                ...
                "agent_framework": acts_config.get(
                    "agent_framework", {"enabled": True}
                ),
                "commit_convention": acts_config.get(
                    "commit_convention", "conventional"
                ),
                "review_provider": acts_config.get("review_provider"),
                "allow_reopen_completed": acts_config.get(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Migrate ACTS 0.6.2 state.json to 1.0.0"
    )
    parser.add_argument("state_json", help="Path to the 0.6.2 state.json file")
    parser.add_argument(
        "--acts-binary",
        dest="acts_binary",
        default=None,
        help="Path to the acts binary (optional, will search PATH if omitted)",
    )
    ...
    acts_binary = find_acts_binary(args.acts_binary)
    migrate_state(state_json_path, acts_binary)
